...
export default function FormCollectionEdit({ collection, products }: Props) {
    // collection.products có thể là mảng string ID hoặc mảng Object đã populate
    const initialSelected = collection.products?.map((p: any) => typeof p === 'object' ? p._id : p) || [];
    const [selectedProducts, setSelectedProducts] = useState<string[]>(initialSelected);
    const [isLoading, setIsLoading] = useState(false);
    const [previewThumb, setPreviewThumb] = useState<string | null>(collection.thumbnail || null);
    const [previewCover, setPreviewCover] = useState<string | null>(collection.cover_image || null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await collectionService.updateCollection(collection._id, formData);
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 space-y-6">
                ...
                <AdminCard title="Cấu hình" subTitle="Vị trí và trạng thái">
                    <div className="space-y-6">
                        <Input label="Thứ tự hiển thị" name="position" type="number" placeholder="1, 2, 3..." defaultValue={collection.position} />
                        ...
                        <div className="space-y-3">
                            ...
                            <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-xl hover:bg-slate-50 transition-colors">
                                <input type="checkbox" name="featured" defaultChecked={collection.featured} className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500" />
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3 p-1 bg-slate-100/50 rounded-2xl border border-slate-200/50">
                                <label className="cursor-pointer">
                                    <input type="radio" name="status" value="active" defaultChecked={collection.status === 'active'} className="sr-only peer" />
                                ...
                                <label className="cursor-pointer">
                                    <input type="radio" name="status" value="inactive" defaultChecked={collection.status === 'inactive'} className="sr-only peer" />
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex items-center justify-end gap-4 p-2">
                    <Button variant="outline" icon={<X className="w-4 h-4" />} type="button" onClick={() => router.push("/admin/collections")}>
                        Hủy bỏ
                    </Button>
